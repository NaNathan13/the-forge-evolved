...
Research answers a question the conversation can't answer on its own. It **gathers and reports** — it never edits code or writes anything outward. Findings go back to whoever called it (usually the `/ponder` session), which decides what to do with them.
...
- **Read-only.** Research gathers; it does not edit code or write task files — that's `/inscribe`'s job.
